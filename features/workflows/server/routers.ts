...
export const workflowsRouter = createTRPCRouter({
    ...
    .mutation(async ({input,ctx}) => {
        const workflow = await prisma.workflow.findUniqueOrThrow({where: {
            id: input.id,
    ...
    updateEditor: protectedProcedure
    .input(z.object({id: z.string(),nodes: z.array(z.object(
        {id: z.string(),
    ...
    .query(async ({ctx,input}: any)=>{
        const workflow :any = await prisma.workflow.findUniqueOrThrow({
            where: {
                id: input.id,
                userId: ctx.auth.user.id
            },
        ...
        const nodes: Node[] =  workflow.nodes.map((node : Node) => ({
            id: node.id,
            type: node.type,
